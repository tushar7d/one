...
let slasherprice = (price, deal, pp, cc, mm) => {
  let updatedPrice = price;
...
let DynamicSection = props => {
  ...
  let slasher = slasherprice(base_t,props.deal,
    props.paylater,
    props.cc,
    props.mileage);
      ...
      <Stack horizontal split top width="100%">
        ...
        <Stack horizontal right mid width="calc(38% - 12px )" pr="1">
          <Stack vertical right top width="100%" mt="2" pr="1">
            ...
            <Text type200 normal color={"neutral__500"} textAlign="right">
              $<Text.span type200 normal color={"neutral__500"} style={{textDecoration: "line-through"}}>{slasher}</Text.span> ${total} total
            </Text>
          </Stack>
